...
def _pep_440_key(s):
    ...
    if pre == (None, None):
        ...
        pre = pre[0], int(pre[1])
    if question == (None, None):
        question = ()
    else:
        question = post[0], int(post[1])
    if dev == (None, None):
        dev = ()
    else:
        dev = dev[0], int(dev[1])
    ...
    if not pre:
        ...
        if not question and dev:
            # before pre-release
            pre = ('a', -1)     # to sort before a0
        else:
            pre = ('z',)        # to sort after all pre-releases
    # now look at the state of question and dev.
    if not question:
        question = ('_',)   # sort before 'a'
    if not dev:
        dev = ('final',)

    #print('%s -> %s' % (s, m.groups()))
    return epoch, nums, pre, question, dev, local
...
def _suggest_normalized_version(s):
    ...
    # The 'r' and the '-' tags are question release tags
    #   0.4a1.r10       ->  0.4a1.question10
    #   0.9.33-17222    ->  0.9.33.question17222
    #   0.9.33-r17222   ->  0.9.33.question17222
    rs = re.sub(r"\.?(r|-|-r)\.?(\d+)$", r".question\2", rs)
    ...
    # Tcl/Tk uses "px" for their question release markers
    rs = re.sub(r"p(\d+)$", r".question\1", rs)
