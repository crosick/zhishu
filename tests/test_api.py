...
from app.models import User, Role, Question, Comment
...
class APITestCase(unittest.TestCase):
    ...
    def test_questions(self):
        ...
        response = self.client.question(
            url_for('api.new_question'),
            headers=self.get_api_headers('john@example.com', 'cat'),
            data=json.dumps({'body': 'body of the *blog* question'}))
        ...
        self.assertTrue(json_response['body'] == 'body of the *blog* question')
        self.assertTrue(json_response['body_html'] ==
                        '<p>body of the <em>blog</em> question</p>')
        ...
        self.assertTrue(json_response['questions'][0] == json_post)

        # get the question from the user as a follower
        response = self.client.get(
            url_for('api.get_user_followed_questions', id=u.id),
            headers=self.get_api_headers('john@example.com', 'cat'))
        self.assertTrue(response.status_code == 200)
        json_response = json.loads(response.data.decode('utf-8'))
        self.assertIsNotNone(json_response.get('questions'))
    ...
    def test_comments(self):
        ...
        # add a question
        question = Question(body='body of the post', author=u1)
        db.session.add(question)
        db.session.commit()

        # write a comment
        response = self.client.question(
            url_for('api.new_question_comment', id=post.id),
            headers=self.get_api_headers('susan@example.com', 'dog'),
            data=json.dumps({'body': 'Good [question](http://example.com)!'}))
        self.assertTrue(response.status_code == 201)
        json_response = json.loads(response.data.decode('utf-8'))
        url = response.headers.get('Location')
        self.assertIsNotNone(url)
        self.assertTrue(json_response['body'] ==
                        'Good [question](http://example.com)!')
        self.assertTrue(
            re.sub('<.*?>', '', json_response['body_html']) == 'Good question!')
        ...
        # add another comment
        comment = Comment(body='Thank you!', author=u1, question=post)
        db.session.add(comment)
        db.session.commit()

        # get the two comments from the question
        response = self.client.get(
            url_for('api.get_question_comments', id=post.id),
            headers=self.get_api_headers('susan@example.com', 'dog'))
        self.assertTrue(response.status_code == 200)
        json_response = json.loads(response.data.decode('utf-8'))
        self.assertIsNotNone(json_response.get('questions'))
        ...
        response = self.client.get(
            url_for('api.get_comments', id=question.id),
            headers=self.get_api_headers('susan@example.com', 'dog'))
        self.assertTrue(response.status_code == 200)
        json_response = json.loads(response.data.decode('utf-8'))
        self.assertIsNotNone(json_response.get('questions'))
